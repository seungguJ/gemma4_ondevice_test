...
- 자동 위임 전에 한 번은 `claude auth login` 또는 적절한 인증 방식으로 인증을 마쳐야 한다.
- Claude가 읽어야 할 프로젝트 파일은 현재 작업 디렉터리 아래에 있어야 한다.
- Codex 샌드박스에서 `CODEX_SANDBOX_NETWORK_DISABLED=1` 상태로 실행하면 `claude` 호출이 `ConnectionRefused`로 실패할 수 있다. 이 경우 네트워크가 허용된 실행 환경에서 `scripts/claude_handoff.sh`를 다시 실행해야 한다.
...
scripts/claude_handoff.sh --task-file docs/handoffs/p006.md --timeout-seconds 900
```
...
- 긴 작업에서는 Claude 출력이 늦게 한 번에 도착할 수 있으므로, 스크립트는 15초 간격 진행 로그와 기본 타임아웃을 함께 사용한다.
