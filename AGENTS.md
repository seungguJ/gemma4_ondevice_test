...
- 온디바이스 AI 모델이 앱에 상주한 채 유지되지 않는지 확인한다. 상시 로드, 선로드 유지, 백그라운드 상주 모델은 금지한다.
- 알림 수신이나 데이터 저장만으로 실시간 AI 분석이 시작되지 않는지 확인한다.
- 정기결제/카드 인사이트 자동 분석은 `24시간 대기 + 충전 중 + 배터리 100% + 모델 미사용` 조건을 모두 만족할 때만 허용한다.
- 사용자가 명시적으로 누른 강제분석 액션만 자동 분석 조건을 우회할 수 있다.
- 추론이 필요한 순간에만 모델을 로드하고 종료 후 `LlmEngine.free()`가 호출되는지 확인한다.
- Worker, 화면, 서비스가 동시에 모델을 잡는 경합 경로가 없는지 확인한다.
- 모델 관련 결정이 바뀌면 `project.md`, 관련 handoff 문서, 모듈 문서를 함께 갱신한다.
